...
- [Invoice Ninja — Invoicing](#-invoice-ninja--invoicing)
...
| **Invoice Ninja** | [invoice.siyalude.io](https://invoice.siyalude.io) | Professional invoicing & billing (Octane) | [./InvoiceNinja-Octane](./InvoiceNinja-Octane) |
| **Infisical** | [infisical.siyalude.io](https://infisical.siyalude.io) | Open source secrets management platform | [./Infisical](./Infisical) |
...
| `calendar_sync_guide.md` | **How to sync Calendar & Contacts (Outlook/Mobile)** |
...
#### 📅 Calendar & Contacts Sync (WebDAV)
Mailu supports full **CalDAV** and **CardDAV** synchronization via Radicale.
*   **Webmail**: Roundcube and SnappyMail are both configured with Calendar and Contacts support.
*   **External Sync**: Works with Outlook (via plugin), iOS, Android, and Thunderbird.
*   **Guide**: See [`Mailu/calendar_sync_guide.md`](./Mailu/calendar_sync_guide.md) for detailed connection steps.
...
**Directory:** [`./InvoiceNinja-Octane`](./InvoiceNinja-Octane)
**URL:** [invoice.siyalude.io](https://invoice.siyalude.io)
**Install Path (server):** `/opt/invoiceninja`

Invoice Ninja is a powerful, open-source invoicing and billing platform. This deployment uses the high-performance Octane engine for faster request handling.
...
| File | Purpose |
|---|---|
| `setup-invoiceninja.sh` | Automated installer: Docker, Nginx, SSL |
| `docker-compose.yml` | App + DB + Redis + PDF Engine definition |
| `.env.example` | Reference environment variables |
...
**Directory:** [`./Infisical`](./Infisical)
**URL:** [infisical.siyalude.io](https://infisical.siyalude.io)
**Install Path (server):** `/opt/infisical`

Infisical is an open-source secret management platform. This deployment uses Docker Compose to run the Infisical backend, PostgreSQL, and Redis. It handles dynamic secret injection, environment variable syncing, and secure storage.

#### Deploy / Re-provision
```bash
scp Infisical/setup-infisical.sh iperamuna:/tmp/
ssh iperamuna "sudo bash /tmp/setup-infisical.sh"
```
...
# Check Server Resource Usage (RAM & Disk per App)
ssh iperamuna "bash /opt/check-docker-usage.sh"
```
...
| Invoice Ninja | `9121` | `127.0.0.1:9121` → Nginx | `invoice.siyalude.io` |
| Infisical | `8080` | `127.0.0.1:8080` → Nginx | `infisical.siyalude.io` |
